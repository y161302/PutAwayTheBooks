...
/**************** 人の画像に関するデータ *****************
  以下の様式に従って、動く人の画像を追加する際は設定をして下さい。
  （動きのない画像に関しては設定の必要がありません。）
  
  1) HumanSetting["ファイル名共通部分"] = { とします。
     中括弧開始 { を忘れずに。
  ...
  2) 各設定値を 設定名:設定値, という形で指定します。
     ...
     行頭の空白（インデント）は見た目をわかりやすくするためにあるので、そろえたほうがいいでしょう。
       frames: [1, 2, 3, 2], は描画順の設定です。
       width: 32, は画像の１フレームの切り取りサイズの幅です。
       height: 48, は画像の１フレームの切り取りサイズの高さです。
       num: 16, は同じ設定のファイル数です。school は16個あるので16です。
  3) 最後に }; と記述して中括弧を閉じます。
     セミコロン;は必ずつけてください。
